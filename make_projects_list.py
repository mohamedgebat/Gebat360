            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {filteredProjects.length === 0 ? (
                ...
                filteredProjects.map(p => {
                  ...
                  const margeEAC = p.revisedBudget > 0 ? ((p.revisedBudget - eac) / p.revisedBudget) * 100 : 0;

                  return (
                    <tr key={p.id} className="hover:bg-slate-50 transition">
                      <td className="p-3 pl-4 font-mono font-bold text-blue-600">{p.code}</td>
                      ...
                      <td className={`p-3 text-right font-extrabold ${margeEAC >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {fmtPct(margeEAC)}
                      </td>
...
with open('src/modules/projects/ProjectsList.tsx', 'w', encoding='utf-8') as f:
    f.write(code)
print("ProjectsList file generated!")
